...
def select_faces(vertices, faces, area, gap_percentage, branch_area = 0.0, cluster_density=3.0):
    ...
    goal_to_keep = (1 - (gap_percentage ** 0.5)) * area - (2.0 * branch_area)  # area of tree to be kept
...
def main_run(vertices, gap_percentage, max_length, shape_name, height, ideal_area,
             branch_area = 0.0, cluster_density=3.0, convex_hull=False):
    ...
    face_keep_indices = select_faces(
        vertices, triangle_faces, ideal_area, gap_percentage, branch_area, cluster_density=cluster_density)
...
def gen_hemisphere(file_name="Hemisphere", vertices_count=20000, gap_percentage=0.136, width=5.0, branch_area = 0.0, cluster_density = 3.0):
    width = float(width)
    ...
    main_run(vertices, gap_percentage, max_length, file_name, width,
             ideal_area_hemisphere, branch_area, cluster_density=cluster_density, convex_hull=True)


def gen_ellipsoid_prolate(file_name="EllProlate", vertices_count=20000, gap_percentage=0.059, height=7.8, radius=5.35, branch_area = 0.0, cluster_density = 3.0):
    height = float(height)
    radius = float(radius)
    ...
    main_run(vertices, gap_percentage, max_length, file_name, height,
             ideal_area_ellipsoid, branch_area, cluster_density=cluster_density, convex_hull=True)


def gen_ellipsoid_oblate(file_name="EllOblate", vertices_count=20000, gap_percentage=0.03, height=5.2, radius=5.7, branch_area = 0.0, cluster_density = 3.0):
    height = float(height)
    radius = float(radius)
    vertices = generate_ellipsoid_vertices(vertices_count, height, radius)
    ...
    main_run(vertices, gap_percentage, max_length, file_name, height,
             ideal_area_ellipsoid, branch_area, cluster_density=cluster_density, convex_hull=False)


def get_cone(file_name="Cone", vertices_count=10000, gap_percentage=0.18, height=8.5, radius=2.5, branch_area = 0.0, cluster_density = 3.0):
    height = float(height)
    radius = float(radius)
    ...
    main_run(vertices, gap_percentage, max_length, file_name,
             height, ideal_area_cone, branch_area, cluster_density=cluster_density)
